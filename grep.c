...
void processing_grep(char** argv, int file_index, struct Flags* flags, char** patterns) {
	...
	if (fp != NULL) {
		...
		while(fgets(string, max_string_size, fp) != NULL) { // считываем построчно - с ограничением в максимальную длину строки
			int to_write = 0;
			for (int i = 0; i < flags->eflag; ++i) {
				regcomp(&pattern, patterns[i], flags->iflag); // iflag должен игнорировать различия регистра если был использован!
				if (!regexec(&pattern, string, 1, pmatch, 0)){
					to_write = 1;
					if (flags->oflag) 
						to_print_oflag(pmatch, pattern, string, flags, string_index, argv[file_index]);
				}
			}
			if (to_write && !flags->vflag) { // выводим на печать если есть в строке и нет флага -v
				to_print(string, flags, string_index, argv[file_index]);
				c_counter++;				
			}
			else if (!to_write && flags->vflag) { // наоборот если в строке нет - и есть флаг -v
				to_print(string, flags, string_index, argv[file_index]);
				c_counter++;				
			}
			string_index++;
		}
		if (flags->lflag && c_counter) { // выводим только название файла если есть совпадение и стоит флаг -l
			printf("%s\n", argv[file_index]);
		} else if (flags->cflag) {
			if (flags->count_files > 1) 
				printf("%s:", argv[file_index]); // выводим на печать имя файла если их больше 1
			printf("%d\n", c_counter);
		} 
	}
}
...
void to_print(char* string, struct Flags *flags, int string_index, char* file_name) {
	if (!flags->cflag && !flags->lflag && !flags->oflag) { // при этих флагах выводить на печать не нужно - здесь чтобы не повторяться
		if (flags->count_files > 1) 
...
void to_print_oflag(regmatch_t *pmatch, regex_t pattern, char* string, struct Flags *flags, int string_index, char* file_name) {
	if (!flags->cflag && !flags->lflag) { // блокируют работу -o флага и выводить на печать ничего не нужно - пусть будут здесь 
		int count = 0;
